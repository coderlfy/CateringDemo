...
{
    public class Order : PageHandlerBase, IHttpHandler
    {
        private EntityOrder _order = new EntityOrder();
        private OrderClient _orderClient = new OrderClient();
            ...
            HttpRequest request)
        {
			...
			this._order.o_ID = paramsarray[OrderMapping.o_ID];
			this._order.orderNO = paramsarray[OrderMapping.orderNO];
			this._order.writeTime = paramsarray[OrderMapping.writeTime];
			this._order.writeIP = paramsarray[OrderMapping.writeIP];
            #endregion
        }
        /// <summary>
        /// 获取分页列表信息
            ...
            ref string json)
        {
            ...
            //json = this._orderClient.GetOrderByPage(
            //    this.SessionUserId, this.SessionUserIp, 
            //    p.PageIndex, p.PageSize, this._order);
            #endregion
        }
        /// <summary>
        /// 获取全部数据
        ...
        {
            #region
            //json = this._orderClient.GetJsonByAll();
            #endregion
        }
        /// <summary>
        /// 添加新记录
        ...
        {
            #region
            //json = this._orderClient.AddOrder(
            //  this.SessionUserId, this.SessionUserIp, 
            //  imapplyforgroup);
            #endregion
        }
