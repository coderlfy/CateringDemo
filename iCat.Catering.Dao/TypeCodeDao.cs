...
{
    public class TypeCodeDao : BaseDao, ITypeCodeDao
    {
        /// <summary>
        /// 保存数据
        /// </summary>
        /// <param name="typeCodeData">欲保存的数据集</param>
        public void Save(TypeCodeData typeCodeData)
        {
            base.Save(typeCodeData);
        }
        ...
        public TypeCodeData SelectSingleT(
            QueryCondition condition)
        {
            return base.GetDataSet<TypeCodeData>(condition);
        }

        #region 用户自定义多表关联查询模版
        /*
        public DataSet SelectTypeCodeByPage(
            QueryCondition condition, 
            out int totalCount) 
        {
            #region
...
join IMGroup d on a.groupId=d.groupId ";
            ...
            condition.AddInKey(TypeCodeMapping.primaryKeyName);
            //填充数据源
