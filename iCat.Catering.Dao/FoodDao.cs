...
{
    public class FoodDao : BaseDao, IFoodDao
    {
        /// <summary>
        /// 保存数据
        /// </summary>
        /// <param name="foodData">欲保存的数据集</param>
        public void Save(FoodData foodData)
        {
            base.Save(foodData);
        }
        ...
        public FoodData SelectSingleT(
            QueryCondition condition)
        {
            return base.GetDataSet<FoodData>(condition);
        }

        #region 用户自定义多表关联查询模版
        /*
        public DataSet SelectFoodByPage(
            QueryCondition condition, 
            out int totalCount) 
        {
            #region
...
join IMGroup d on a.groupId=d.groupId ";
            ...
            condition.AddInKey(FoodMapping.primaryKeyName);
            //填充数据源
