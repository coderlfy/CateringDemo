...
namespace iCat.Catering.Dao
{
    public class OrderDetailDao : BaseDao, IOrderDetailDao
    {
        ...
        /// 保存数据
        /// </summary>
        /// <param name="orderDetailData">欲保存的数据集</param>
        public void Save(OrderDetailData orderDetailData)
        {
            base.Save(orderDetailData);
        }
        ...
        /// <returns>OrderDetail的数据集</returns>
        public OrderDetailData SelectSingleT(
            QueryCondition condition)
        {
            return base.GetDataSet<OrderDetailData>(condition);
        }

        #region 用户自定义多表关联查询模版
        /*
        public DataSet SelectOrderDetailByPage(
            QueryCondition condition, 
            out int totalCount) 
        {
            #region
...
join IMGroup d on a.groupId=d.groupId ";
            ...
            condition.AddInKey(OrderDetailMapping.primaryKeyName);
            //填充数据源
